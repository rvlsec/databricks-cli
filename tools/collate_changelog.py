...
# ///
"""Collate ``changelog.d/`` fragments into ``NEXT_CHANGELOG.md``.

Each PR adds its own file under ``changelog.d/<section>/`` instead of editing
...
# A PR reference such as ``#1234`` anywhere in an entry.
PR_REF_RE = re.compile(r"#\d+")

# Leading run of digits in a fragment filename, e.g. ``5464`` in
# ``5464-quickstart.md``; used to derive the PR reference.
...
def pr_number_from_name(path):
    """Return the PR number from a fragment filename, or None.

    The PR number is the leading run of digits in the filename, so both the
    bare ``5464.md`` and the descriptive ``5464-quickstart.md`` work.

    >>> pr_number_from_name(pathlib.Path("changelog.d/cli/5464.md"))
    '5464'
    >>> pr_number_from_name(pathlib.Path("changelog.d/cli/5464-quickstart.md"))
    '5464'
    >>> pr_number_from_name(pathlib.Path("changelog.d/cli/quickstart.md")) is None
    True
    """
    match = LEADING_NUMBER_RE.match(path.stem)
    return match.group(0) if match else None


def ensure_pr_reference(text, pr_number):
    """Append a ``(#pr_number)`` reference to *text* unless one is present.

    The reference is inserted before a single trailing period to match the
    changelog convention ``... change (#1234).``. An entry that already cites a
    PR is left alone, so an author can reference a different or additional PR.

    >>> ensure_pr_reference("* Added a flag.", "1234")
    '* Added a flag (#1234).'
    >>> ensure_pr_reference("* Added a flag", "1234")
    '* Added a flag (#1234)'
    >>> ensure_pr_reference("* Added a flag (#1234).", "1234")
    '* Added a flag (#1234).'
    >>> ensure_pr_reference("* Reverts #1230.", "1234")
    '* Reverts #1230.'
    """
    if PR_REF_RE.search(text):
        return text
    if text.endswith(".") and not text.endswith(".."):
        return f"{text[:-1]} (#{pr_number})."
    return f"{text} (#{pr_number})"
...
def find_misplaced(changelog_dir):
    """Return fragment paths that are not ``changelog.d/<section>/<name>.md``."""
    misplaced = []
...
def collate(root):
    ...
    for slug, header in SECTIONS:
        ...
        for path in sorted(section_dir.glob("*.md")):
            if path.name == "README.md":
                continue
            entry = normalize_entry(path.read_text(encoding="utf-8"))
            pr_number = pr_number_from_name(path)
            if pr_number is not None:
                entry = ensure_pr_reference(entry, pr_number)
